...
import { users, companies, products, scans, declarations, ruleEvaluations, complaints, companyResponses, governmentReviews, inspections, auditLogs, } from './db/schema.js';
import { eq, desc, sql } from 'drizzle-orm';
import bcrypt from 'bcryptjs';
import { ComplianceEvaluator, LEGAL_METROLOGY_RULES, CURRENT_RULESET_VERSION } from '../../../packages/compliance-engine/src/index';
...
async function start() {
    ...
    server.get('/api/products', async (request, reply) => {
        ...
        if (query.search) {
            ...
            allProducts = allProducts.filter((p) => p.name.toLowerCase().includes(s) || p.brand.toLowerCase().includes(s));
        }
    ...
    server.post('/api/scans/analyze', async (request, reply) => {
        ...
        let ocrOverride = undefined;
        let packageHeightMm = undefined;
        let packageWidthMm = undefined;
        ...
        if (request.isMultipart()) {
            ...
            for await (const part of parts) {
                if (part.type === 'file') {
                    ...
                    if (fieldName === 'ocrText')
                        ocrOverride = val;
                    if (fieldName === 'packageHeightMm')
                        packageHeightMm = parseFloat(val);
                    if (fieldName === 'packageWidthMm')
                        packageWidthMm = parseFloat(val);
                }
            }
        }
        else {
            ...
            if (body.ocrText)
                ocrOverride = body.ocrText;
            if (body.packageHeightMm)
                packageHeightMm = body.packageHeightMm;
            if (body.packageWidthMm)
                packageWidthMm = body.packageWidthMm;
        }
...
`.trim();
        ...
        for (const [type, dec] of Object.entries(analysis.declarations)) {
            db.insert(declarations)
                ...
                label: dec.label,
                detectedValue: dec.detectedValue,
                confidence: dec.confidence,
                rawSnippet: dec.rawSnippet || null,
    ...
    server.post('/api/complaints', async (request, reply) => {
        ...
        db.run(sql `UPDATE companies SET active_complaints_count = active_complaints_count + 1 WHERE id = ${finalCompanyId}`);
        // Increment product issues count if product exists
        if (finalProductId) {
            db.run(sql `UPDATE products SET potential_issues_count = potential_issues_count + 1 WHERE id = ${finalProductId}`);
        }
        // Audit Log
        db.insert(auditLogs)
            .values({
    ...
    server.post('/api/complaints/:id/verify', async (request, reply) => {
        ...
        db.insert(governmentReviews)
            ...
            .run();
        const newStatus = body.decision === 'VERIFIED_VIOLATION'
            ? 'VERIFIED'
        ...
        if (body.decision === 'VERIFIED_VIOLATION') {
            ...
            if (complaint.productId) {
                db.run(sql `UPDATE products SET verified_violations_count = verified_violations_count + 1 WHERE id = ${complaint.productId}`);
            }
            // Recalculate company risk
            db.run(sql `UPDATE companies SET risk_score = MIN(100, risk_score + 5), risk_level = CASE WHEN risk_score + 5 > 70 THEN 'HIGH' WHEN risk_score + 5 > 40 THEN 'MEDIUM' ELSE 'LOW' END WHERE id = ${complaint.companyId}`);
        }
        // Audit log
        db.insert(auditLogs)
            .values({
