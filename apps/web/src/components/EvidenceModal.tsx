...
import { X, ShieldAlert, Info, Hash, FileText } from 'lucide-react';
import { RuleEvaluation, ExtractedDeclaration, BoundingBox, ImageIntegrityMetadata } from '../types';
...
export const EvidenceModal: React.FC<EvidenceModalProps> = ({
  ...
  const boxes: BoundingBox[] =
    rule?.evidenceRegions && rule.evidenceRegions.length > 0
      ? rule.evidenceRegions
      : declaration?.evidenceRegions && declaration.evidenceRegions.length > 0
      ? declaration.evidenceRegions
      : rule?.evidenceBox
      ? [rule.evidenceBox]
      : declaration?.evidenceBox
      ? [declaration.evidenceBox]
      : [];
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Legal Metrology (Packaged Commodities) Rules, 2011 • Rule Version 2026.1
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex flex-col items-center justify-center bg-slate-950 rounded-xl p-3 border border-slate-800">
              <div className="flex items-center justify-between w-full mb-2 px-1">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
                  Scanned Package Panel {boxes.length > 0 ? `(${boxes.length} Region${boxes.length > 1 ? 's' : ''})` : ''}
                </span>
                {imageIntegrity?.sha256Hash && (
                  <span className="text-[10px] text-emerald-400 font-mono flex items-center gap-1">
                    <Hash className="w-3 h-3" />
                    {imageIntegrity.sha256Hash.substring(0, 8)}...
                  </span>
                )}
              </div>
              <div className="relative w-full max-h-[420px] overflow-hidden rounded-lg flex items-center justify-center bg-slate-900 p-2">
                <div className="relative inline-block max-h-[400px]">
                  ...
                  />
                  {boxes.map((box, idx) => {
                    if (!imageDimensions || imageDimensions.width <= 0 || imageDimensions.height <= 0) return null;
                    return (
                      <div
                        key={idx}
            ...
            <div className="space-y-4">
              <div>
                <span className="text-xs font-bold text-blue-600 uppercase tracking-wider">
                  Target Declaration / Statutory Rule
                </span>
                ...
                <p className="text-xs text-slate-500 font-medium mt-0.5 flex items-center gap-1">
                  ...
                  {rule?.officialSource || rule?.sectionReference || 'Rule 6 Legal Metrology PC Rules, 2011'}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
                  <span className="text-xs text-slate-500 block">Evidence Confidence</span>
          ...
          <div className="p-3.5 bg-amber-50 rounded-xl border border-amber-200 flex items-start gap-2.5">
            ...
            <p className="text-xs text-amber-900">
              <strong className="font-semibold">Statutory Advisory:</strong> This AI screening assessment is generated for preliminary risk-screening and grievance assistance under Legal Metrology Rules (v2026.1). It does not constitute a final judicial determination under the Legal Metrology Act, 2009. Official enforcement or compounding notice requires physical verification by an authorized Legal Metrology Officer.
            </p>
          </div>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-3.5 bg-slate-50 rounded-b-2xl border-t border-slate-200 text-xs text-slate-500">
          <span>Rule Set Version: <strong>2026.1</strong></span>
          <button
            onClick={onClose}
