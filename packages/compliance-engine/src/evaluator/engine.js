...
export class ComplianceEvaluator {
    static evaluateDeclarations(declarations, fontAnalysis) {
        const results = [];
        // Rule 1: Manufacturer
    ...
    static analyzeScan(rawOcrText, calibration) {
        ...
        for (const res of ruleResults) {
            if (res.status === 'COMPLIANT')
                compliantCount++;
            else if (res.status === 'POTENTIAL_NON_COMPLIANCE')
                flaggedCount++;
            else
                reviewCount++;
        }
        ...
        if (flaggedCount >= 2 && score > 65)
            score = 65;
        if (flaggedCount >= 3 && score > 45)
            score = 45;
        ...
        const summary = {
            ...
            disclaimer: 'AI screening assistance only. Not a formal judicial or enforcement order under the Legal Metrology Act, 2009. Official action is subject to physical verification by authorized Legal Metrology Officers.',
        };
